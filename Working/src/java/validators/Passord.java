...
public class Passord implements Validator{
    ...
    public void validate(FacesContext fc, UIComponent uic, Object o) throws ValidatorException {
        ...
        if(!size){
            FacesMessage message = language.Messages.getMessage("language.auth", "password_short");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
            throw new ValidatorException(message);
        }
        if(!spesialtegn){
            FacesMessage message = language.Messages.getMessage("language.auth", "password_special");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
            throw new ValidatorException(message);
        }
        if(!tall){
            FacesMessage message = language.Messages.getMessage("language.auth", "password_number");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
            throw new ValidatorException(message);
        }
